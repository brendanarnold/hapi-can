...
const from = (obj) => {
  ...
  const repositoryItems = isRequest 
    ? server.app['hapi-can'].repositoryItems.request
    : server.app['hapi-can'].repositoryItems.server
  const h = isRequest
  ...
  const get = async (name, ...extraArgs) => {
    if (!name || typeof cache[name] === 'undefined') {
      throw new Error('get requires a valid name to lookup')
    }
